...
for i = 1:m,
  ...
  yt(1, y(i)) = 1;
  t = -((yt .* log(h2(i,:))) + ((1 - yt).*log(1-h2(i,:))));
  J = J + sum(t);  
end;
J = J/m;

% Adding Regualrization to the cost function
Theta1_r = Theta1(:, 2:end);
Theta2_r = Theta2(:, 2:end);

J = J + (lambda/(2*m))*(sum(sum(Theta1_r .* Theta1_r)) + sum(sum(Theta2_r .* Theta2_r)));
% -------------------------------------------------------------
...
for i = 1:m,
  ...
  d2 = d2(2:end);
  Theta1_grad = Theta1_grad + (d2 * Xt(i,:));
  Theta2_grad = Theta2_grad + (d3 * [1 a2(i,:)]);
end
